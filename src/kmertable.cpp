...
size_t KmerTable::parseRead(FastQRecord &record, vector<Kmer> *kmerBuffer)
{
...
void KmerTable::workerThread(size_t thisThread, FastQReader& inputs)
{
        ...
        while (true) {
                ...
                if (myKmerBuf.size() == 0) {
                        ...
                        size_t chunkID;
                        inputs.getNextChunk(myReadBuf, chunkID);
...
void KmerTable::parseInputFiles(LibraryContainer &inputs)
{
        ...
        for (const Library& lib : inputs) {
                ...
                for (size_t i = 0; i < workerThreads.size(); i++)
                        workerThreads[i] = thread(&KmerTable::workerThread,
                                                  this, i, ref(myReader));
