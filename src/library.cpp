...
void ReadLibrary::writeMetadata() const
{
        ...
        numReads(0), avgReadLength(0.0)
{
        ...
        if (inputFilename.length() >= 3)
                extension = inputFilename.substr(inputFilename.length() - 3);
        transform(extension.begin(), extension.end(), extension.begin(), ::toupper);
        if (extension == ".FA") {
                fileType = FASTA;
                baseFilename = inputFilename.substr(0, inputFilename.length() - 3);
        ...
        if (fileType == UNKNOWN_FT) {
                string msg = "don't know how to open file: \"" + inputFilename +
                             "\"\nExpected one of the following extensions: "
                             ".fastq, .fasta, .sam, .raw (or .gz variants thereof)\n";
...
LibraryContainer::LibraryContainer(const std::string& readFilename)
{
        try {
                // option A: the read file contains reads
                insert(ReadLibrary(readFilename, "", ""));
        } catch (exception& e) {
                // option B: the read file is a manifest file
...
void LibraryContainer::inputThreadLibrary(ReadLibrary& input)
{
        ...
        while (readFile->good()) {
                ...
                // don't push empty blocks onto the worker queue
                if (recordBuffer.empty()) {
                        // first restore the inputBlock
                        lock_guard<mutex> inputLock(inputMutex);
                        inputBlocks.push_back(block);
                        break;
                }
        ...
        cout << "Number of reads processed: " << totNumReads << " ("
             << Util::humanTime(Util::stopChrono()) << ")" << endl;
...
void LibraryContainer::readMetadata()
{
        for (auto& it : container)
                it.readMetadata();
}

void LibraryContainer::writeMetadata() const
{
